...
# الترحيل (000 حتى 022) بالترتيب الرقمي من الصفر. الحزمة مكتفية ذاتيًا
...
echo "=== التحقق من اكتمال جميع ملفات الترحيل المطلوبة (000-022) قبل أي تنفيذ ==="
missing=0
...
echo "=== نجاح: جميع ملفات الترحيل الـ23 (000-022) موجودة فعليًا في الحزمة ==="
...
echo "=== تطبيق كل ملفات الترحيل بالترتيب الرقمي (000 حتى 022) ==="
for f in "${REQUIRED_FILES[@]}"; do
  echo "--- تطبيق: $f ---"
...
echo "=== نجاح: قاعدة الاختبار '$DB_NAME' جاهزة بكل الجداول من الصفر (23 ملف ترحيل مُطبَّق) ==="
