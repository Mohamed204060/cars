...
# الترحيل (000 حتى 021) بالترتيب الرقمي من الصفر. الحزمة مكتفية ذاتيًا
...
REQUIRED_FILES=(
  ...
  "021_trm_unified_ratings.sql"
)

echo "=== التحقق من اكتمال جميع ملفات الترحيل المطلوبة (000-021) قبل أي تنفيذ ==="
missing=0
...
echo "=== نجاح: جميع ملفات الترحيل الـ22 (000-021) موجودة فعليًا في الحزمة ==="
...
echo "=== تطبيق كل ملفات الترحيل بالترتيب الرقمي (000 حتى 021) ==="
for f in "${REQUIRED_FILES[@]}"; do
  echo "--- تطبيق: $f ---"
...
echo "=== نجاح: قاعدة الاختبار '$DB_NAME' جاهزة بكل الجداول من الصفر (22 ملف ترحيل مُطبَّق) ==="
