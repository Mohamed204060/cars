...
REQUIRED_FILES=(
  ...
  "023_iam_sessions.sql" "025_sys_idempotency_keys.sql"
)

echo "=== التحقق من اكتمال جميع ملفات الترحيل المطلوبة (000-025) قبل أي تنفيذ ==="
missing=0
...
echo "=== نجاح: جميع ملفات الترحيل الـ25 (000-025، بلا 024) موجودة فعليًا في الحزمة ==="
...
echo "=== تطبيق كل ملفات الترحيل بالترتيب الرقمي (000 حتى 025) ==="
for f in "${REQUIRED_FILES[@]}"; do
  echo "--- تطبيق: $f ---"
...
echo "=== نجاح: قاعدة الاختبار '$DB_NAME' جاهزة بكل الجداول من الصفر (25 ملف ترحيل مُطبَّق) ==="
